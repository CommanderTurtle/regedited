...
```
src/
├── main.rs           # CLI: 48+ commands via clap (2,166 lines)
├── lib.rs            # Core types, re-exports, 21 modules
│
├── CORE ENGINE
...
│                       # Zero-allocation case-insensitive byte search (find_case_insensitive)
...
├── zone_type.rs      # ZoneType enum (Markdown/Code/Media/Database) + hex-word codec TxLLLLLLL
│
├── WINDOWS-NATIVE (docs.shel.sh/xml-project — runtime: cmd.exe)
├── echo.rs           # 5 echo variants: str(1)/realstr(2)/realrealstr(3)/literal(4)/actual(5)
│                       # Native cmd.exe batch. NOT PowerShell — type `cmd` in PS first.
├── clip.rs           # Cross-platform clipboard: 6 commands via arboard crate
│                       # Native Win32 API / NSPasteboard / X11+Wayland
├── utf16.rs          # getutf() DWORD-style line number encoding/decoding utility
│
├── encapsulate.rs    # Three-mode system for C-assembly : b=["..."] c=['...'] d=["'...'"]
...
The hex-word format `TxLLLLLLL` packs type and line number into a u32. This is parsed via bit masking, not string parsing, for speed.
...
|--------------------|---------|---------|-------|
| +0 | `## SECTION: <Name>` | `## SECTION: CodeSnippets` | Header marker |
| +1 | `index: <N>` | `index: 200` | Human-readable index |
| +2 | Hex-Word Store | `0x0000000 : ...` | 6 hex-words, colon-separated |
...
| GET | `/section/{name}/ascii` | Hex-word store |
