...
```
src/
├── main.rs           # CLI: 40+ commands via clap
...
│
├── SERIOUS SUBSTRATE
├── wal.rs            # Write-ahead log (crash-safe writes)
├── transaction.rs    # Begin/commit/rollback batch edits
├── schema.rs         # Optional per-section type enforcement
├── typed_value.rs    # REG_SZ/DWORD/QWORD/BINARY/JSON/TOML
└── serve.rs          # HTTP server (registry container mode)
```
...
| +2 | Hex-Word Store | `0x00000000 : ...` | 6 hex-words, colon-separated |
