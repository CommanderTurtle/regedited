...
fn cmd_convert(
    ...
    println!("  {} : {} : 0x00000000 : 0x00000000 : 0x00000000 : 0x00000000",
        start_hex, end_hex
    );
...
fn cmd_types() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  0x0000000A = Markdown, line 10");
    println!("  0x10000050 = Code, line 80");
    println!("  0x20000A00 = Media, line 2560");
    println!("  0x30000001 = Database, line 1");
    println!();
        ...
        for (sec_name, sec_schema) in &schema.sections {
            if let Some(info) = header.get_section(sec_name) {
                ...
                if info.numeric_line < lines.len() {
                    let nums: Vec<&str> = lines[info.numeric_line].split('\t').collect();
                    for (i, v) in nums.iter().enumerate() {
                        values.insert(format!("num_{}", i), v.to_string());
                    }
                }
