        ...
        match self {
            ...
            SchemaFieldType::Boolean => {
                ...
                if v != "true" && v != "false" && v != "1" && v != "0" && v != "yes" && v != "no" {
                    return Err(RegeditedError::Parse(format!(
                        "'{}' is not a valid boolean (true/false/1/0)",
                        value
                    )));
                }
                Ok(())
            ...
            SchemaFieldType::Hex => {
                if !value.starts_with("0x") && !value.starts_with("0X") {
                    return Err(RegeditedError::Parse(format!(
                        "'{}' is not valid hex (must start with 0x)",
                        value
                    )));
                }
                Ok(())
...
---"#;
        ...
        assert!(matches!(
            max_size.constraint,
            FieldConstraint::Range(1, 1000000)
        ));
    }
    ...
    fn test_schema_validate() {
        ...
        sec.add_field(SchemaField::new(
            "mode",
            SchemaFieldType::Enum(vec!["auto".to_string(), "manual".to_string()]),
        ));
