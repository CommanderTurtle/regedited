...
pub fn fast_scan_content(content: &str) -> Result<Vec<ScannedSection>> {
    ...
    for (name, info) in &header.sections {
        ...
        // Read index (line before ASCII store)
        let index = if info.header_line + 1 < lines.len() {
            lines[info.header_line + 1].trim().parse::<u64>().unwrap_or(0)
        } else {
            0
        };
        ...
        // Read database values (9 tab-separated numbers)
...
/// Parse 9 tab-separated numbers quickly (no error handling overhead)
