...
impl Transaction {
    ...
    pub fn begin<P: AsRef<Path>>(doc_path: P) -> Result<Self> {
        ...
        if Wal::exists_for(&doc_path) && !Wal::is_committed(&doc_path)? {
            return Err(RegeditedError::Parse(
                format!("Transaction already in progress for {}. Run 'tx rollback' or 'tx commit' first.",
                    doc_path.display())
            ));
        }
    ...
    pub fn stage_set_num(&mut self, section: &str, index: usize, old_value: i64, new_value: i64) -> Result<()> {
    ...
    pub fn stage_set_str(&mut self, section: &str, index: usize, old_value: &str, new_value: &str) -> Result<()> {
    ...
    /// Stage a zone change
    pub fn stage_set_zone(&mut self, section: &str, zone: usize,
                          old_range: (u32, u32, String), new_range: (u32, u32, String)) -> Result<()> {
    ...
    pub fn stage_zone_replace(&mut self, section: &str, zone: usize, old_content: &str, new_content: &str) -> Result<()> {
    ...
    fn stage(&mut self, op: WalOperation) -> Result<()> {
        ...
        self.staged.push(StagedOperation { seq, op, description });
    ...
    pub fn commit(&mut self) -> Result<Vec<WalOperation>> {
        ...
        if !matches!(self.state, TransactionState::Started | TransactionState::Staging) {
            return Err(RegeditedError::Parse(
...
impl TransactionManager {
    ...
    pub fn begin<P: AsRef<Path>>(&mut self, doc_path: P) -> Result<&Transaction> {
        ...
        if self.active.contains_key(&path) {
            return Err(RegeditedError::Parse(
                format!("Transaction already active for {}", path)
            ));
        }
    ...
    pub fn commit<P: AsRef<Path>>(&mut self, doc_path: P) -> Result<Vec<WalOperation>> {
        ...
        if let Some(tx) = self.active.get_mut(&path) {
            ...
            Err(RegeditedError::Parse(
                format!("No active transaction for {}", path)
            ))
        }
    }
