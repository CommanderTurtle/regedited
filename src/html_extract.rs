...
pub fn format_as_encapsulated(extracts: &[HtmlExtract], mode: EncapMode) -> Vec<String> {
    extracts
        .iter()
        .map(|e| encapsulate(&e.value, mode))
        .collect()
}
...
/// # → -1=["https://another.com"]
/// ```
pub fn format_numbered(extracts: &[HtmlExtract], mode: EncapMode) -> Vec<String> {
    extracts
        .iter()
        .enumerate()
        .map(|(i, e)| {
...
pub fn format_with_counter(extracts: &[HtmlExtract], mode: EncapMode) -> Vec<String> {
    extracts
        .iter()
        .enumerate()
        .map(|(i, e)| {
