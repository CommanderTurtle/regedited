...
/// ```
pub fn clip_hexword_range(start: u32, end: u32, zone_type: crate::zone_type::ZoneType) -> Result<String> {
...
pub fn clip_zone_content(
    ...
    if zone_index >= 3 {
        return Err(RegeditedError::Parse(
            format!("Zone index {} out of range (0-2)", zone_index)
        ));
    }
...
pub fn clip_db_value(
    ...
    if value_index >= 9 {
        return Err(RegeditedError::Parse(
            format!("DB value index {} out of range (0-8)", value_index)
        ));
    }
...
/// Copy the ASCII store line to clipboard
///
/// # Example
/// ```bash
