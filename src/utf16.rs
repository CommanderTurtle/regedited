...
//!
//! The hex-word line store uses UTF-16LE encoding to store line number pairs.
//! This enables:
...
//! ## Format
//!
//! The hex-word line contains 6 UTF-16LE code units (12 bytes total):
...
pub fn decode_utf16le_bytes(bytes: &[u8]) -> Result<u32> {
    if bytes.len() < 4 {
        return Err(RegeditedError::Parse(format!(
            "Fixed-width decode needs 4 bytes, got {}",
            bytes.len()
        )));
    }
...
pub fn getutf_decode(input: &str) -> Result<u32> {
    ...
    let hex_bytes: Vec<u8> = cleaned
        .split_whitespace()
        .filter(|s| s.len() == 2) // Only valid 2-char hex bytes
        .map(|s| u8::from_str_radix(s, 16))
        .collect::<std::result::Result<Vec<_>, _>>()
        .map_err(|_| {
            RegeditedError::Parse(format!("Cannot parse '{}' as hex or decimal", input))
        })?;
    ...
    if hex_bytes.len() >= 4 {
        ...
        Err(RegeditedError::Parse(format!(
            "Cannot parse '{}' as hex or decimal",
            input
        )))
    }
}
