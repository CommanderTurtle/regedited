...
impl TypedValue {
    ...
    pub fn from_store_string(s: &str, type_name: &str) -> Result<Self> {
        match type_name {
            ...
            _ => Err(RegeditedError::Parse(format!("Unknown type: {}", type_name))),
        }
    }
    ...
    pub fn display(&self) -> String {
        match self {
            ...
            TypedValue::Json(v) => format!("JSON: {}", v.to_string().chars().take(60).collect::<String>()),
...
fn parse_hex_string(s: &str) -> Result<Vec<u8>> {
    ...
    if s.contains(' ') {
        ...
        for part in parts {
            let byte = u8::from_str_radix(part, 16)
                .map_err(|e| RegeditedError::Parse(format!("Invalid hex byte '{}': {}", part, e)))?;
            bytes.push(byte);
        }
        return Ok(bytes);
    }
    ...
    let clean = s.chars().filter(|c| c.is_ascii_hexdigit()).collect::<String>();
    if clean.len() % 2 != 0 {
        return Err(RegeditedError::Parse("Hex string has odd length".to_string()));
    }
    let mut bytes = Vec::new();
...
fn hex_encode_preview(bytes: &[u8]) -> String {
    let preview: Vec<String> = bytes.iter().take(16)
        .map(|b| format!("0x{:02X}", b))
        .collect();
    if bytes.len() > 16 {
        format!("{} ... ({} more bytes)", preview.join(" "), bytes.len() - 16)
    } else {
        preview.join(" ")
    }
}
