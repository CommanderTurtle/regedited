...
//! 3|1705312201|set-zone|Code|0|0x0000000:0x0000000|0x10000020:1x0000050|c9a5b3d7
//! ---
    ...
    fn serialize_body(&self) -> String {
        match self {
            ...
            WalOperation::SetStr {
                ...
                format!(
                    "set-str|{}|{}|{}|{}",
                    section, index, old_escaped, new_escaped
                )
            }
            WalOperation::SetZone {
                section,
                zone,
                old_range,
                new_range,
            ...
            WalOperation::ZoneReplace {
                ...
                format!(
                    "zone-replace|{}|{}|{}|{}",
                    section, zone, old_escaped, new_escaped
                )
            }
        }
    }
        ...
        match parts[0].as_str() {
            ...
            "set-zone" if parts.len() >= 5 => {
                let parse_range = |s: &str| -> Result<(u32, u32, String)> {
                    ...
                    if rp.len() >= 3 {
                        Ok((
                            rp[0].parse().unwrap_or(0),
                            rp[1].parse().unwrap_or(0),
                            rp[2].to_string(),
                        ))
                    } else {
    ...
    pub fn description(&self) -> String {
        match self {
            WalOperation::SetNum {
                ...
                format!(
                    "set-num [{}].{}: {} -> {}",
                    section, index, old_value, new_value
                )
            }
            WalOperation::SetStr {
                section,
...
impl WalEntry {
    ...
    pub fn to_line(&self) -> String {
        ...
        format!(
            "{}|{}|{}|{:08x}",
            self.seq, self.timestamp, body, self.checksum
        )
    }
...
impl Wal {
    ...
    pub fn read_entries<P: AsRef<Path>>(doc_path: P) -> Result<Vec<WalEntry>> {
        ...
        for line_result in reader.lines() {
            ...
            if trimmed.starts_with(WAL_COMMIT)
                || trimmed.starts_with('#')
                || trimmed.starts_with(WAL_HEADER)
            {
                continue;
            }
...
mod tests {
    ...
    fn test_wal_operation_set_str_with_pipes() {
        ...
        let deserialized =
            WalOperation::deserialize_body("set-str|Config|0|a\\|b\\|c|x\\|y\\|z").unwrap();
        match deserialized {
            WalOperation::SetStr {
                old_value,
                new_value,
